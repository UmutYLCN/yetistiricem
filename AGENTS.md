...
- Data model: a `StudyCamp` owns its branches (`SubjectPlaylist`; `subject` = branch name), its tempo (`CampSchedule`, auto or manual `weekPlan`) and its shift events; screens show the open camp via `buildCampSchedule`. Tempo is per camp only (edited with "Tempoyu düzenle", never in Ayarlar). Data updates go through `src/lib/plannerOps.ts`; wizard rules in `src/lib/campDraft.ts`. Product notes and V1 boundaries: `docs/camp-creation-wizard-notes.md`.
