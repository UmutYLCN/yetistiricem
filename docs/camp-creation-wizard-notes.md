...
- **Camp** (`StudyCamp`): the whole study program, e.g. "TYT 2027". It owns its branches, its **tempo** (`CampSchedule`: dates, daily time, weekly form) and its shift events. Several camps can exist; the plan screens show the **open camp** (`yt_active_camp`), chosen from the compact sidebar switcher (shown with two or more camps), the mobile top bar or the Kamplar page. With two or more camps the switcher also offers **Tüm Kamplar** (the default until the user picks one camp): Bugün, Haftalık and İlerleme then combine every camp with videos by date, each keeping its own tempo (see [`planner-engine.md`](planner-engine.md)). Kamplar still manages one real camp; there "Yönet" picks it without leaving the combined view. The plan screens carry no camp/tempo summary bar.
...
1. **Kaynaklar**: three illustrated ways (`SourcePicker`). **Oynatma listesi**: paste a YouTube playlist (read by the playlist endpoint with real titles, order, channels and durations, reviewed before adding); each playlist becomes its own editable branch card: branch name (suggested from the title by `guessBranchName`, e.g. "2027 TYT Matematik Kampı" → Matematik), source name, colour, and the ordered videos (removable). **Videolar**: paste video links, one or more per line; the videos endpoint reads their titles, channels and durations for the same review, and they go to a new branch or an existing one. **Elle ekle**: lessons from outside YouTube, typed as topics with a duration (Enter adds the next, pasted lines split into topics); they become a new, named branch of link-less videos, whose links can be added later from the plan. Demo templates are no longer offered here (the demo camp still uses them). The branch list appears once there is a branch; before that only the "add at least one" error can show. Continue needs at least one branch, every branch named and non-empty.
