...
Yerel geliştirme .env.local dosyasını Vite tarafından okur. Üretimde anahtarı barındırma sağlayıcısının gizli ortam değişkeni olarak tanımla. Statik dosyaları tek başına yükleyen bir barındırma hizmeti endpoint’i çalıştırmaz; playlist içe aktarımı için Node sunucusunu da çalıştırmalıdır.
